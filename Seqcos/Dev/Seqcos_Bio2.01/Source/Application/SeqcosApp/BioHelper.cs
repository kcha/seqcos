...
namespace SeqcosApp
{
    ...
    public static class BioHelper
    {
        ...
        /// <summary>
        /// Returns sequence from a sequence object as string
        /// </summary>
        /// <param name="seqObj">A Sequence object</param>
        /// <returns>Sequence string</returns>
        public static string GetStringSequence(ISequence seqObj)
        {
            char[] sequence = new char[seqObj.Count];
            for (long index = 0; index < seqObj.Count; index++)
            {
                sequence[index] = (char)seqObj[index];
            }
